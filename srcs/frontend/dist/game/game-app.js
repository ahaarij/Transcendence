...
export class GameApp {
    ...
    startTournament() {
        ...
        for (let i = 1; i <= this.tournamentSize; i++) {
            const input = this.container.querySelector(`#player${i}`);
    ...
    prepareNextMatch() {
        if (this.currentMatchIndex < this.tournamentBracket.length) {
            ...
            this.container.querySelector("#tourneyRoundDisplay").innerText = this.tournamentRound.toString();
            this.container.querySelector("#matchupText").innerText = `${match.player1}  VS  ${match.player2}`;
            ...
            if (this.tournamentBracket.length === 1) {
                this.container.querySelector("#championName").innerText = this.tournamentWinner[0];
                this.championScreen.style.display = "block";
                this.tournamentMatchScreen.style.display = "none";
                this.gameOverScreen.style.display = "none";
                this.uiLayer.style.display = "flex";
    ...
    predictBallY(ball, velocity, aiPaddle) {
    ...
    renderBracket() {
        const container = this.container.querySelector('#bracketContainer');
        container.innerHTML = '';
    ...
    createBracketColumn(matches, roundIdx, side) {
    ...
    createMatchBox(match, roundIdx, matchIdx) {
        ...
        if (match.winner === match.p1 && match.winner)
            p1.classList.add('winner');
        const p2 = document.createElement('div');
        p2.className = 'player-slot';
        p2.innerText = match.p2 || 'TBD';
        if (match.winner === match.p2 && match.winner)
